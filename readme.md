...
##  Файлы проекта
В репозитории 3 файла, но работает только 1:

 covid_etl_dag.py — основной рабочий пайплайн (Pandas)
   - RAW: загрузка CSV → PostgreSQL
   - STAGE: трансформация Wide → Long (797K строк)
   - MART: агрегация по месяцам (~5K записей)
…- Приоритет: работающее решение > идеальное но нерабочее


transform_stage.py, aggregate_mart.py — Spark версии
   - Написаны как альтернатива для масштабирования
   - В текущей реализации не активны (закомментированы в DAG)
   - Могут быть активированы при необходимости

Почему так:
- Для тестового объёма данных Pandas справляется за 5-7 минут
- Spark требует дополнительной настройки кластера
- Приоритет: работающее решение > идеальное но нерабочее

Готов показать работающий пайплайн на созвоне.
